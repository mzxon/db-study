...
select ename, job, sal, comm, sal*12+comm as 연봉 from emp;

문24) 커미션이 null이면 0으로 바꾼후 연봉을 다시 계산해서
      이름, 급여, 커미션, 연봉을 조회하시오
select ename, job, sal, comm, sal*12+nvl(comm, 0) as 연봉
from emp;
...
문29) 다음의 SQL문을 분석하시오
      ...
      order by sal;
급여가 200보다 많지 않고 300보다 작지 않은 직원의 사번과 급여 검색하기(급여기준)
...
문30) 다음의 SQL문을 분석하시오
      ...
      where not sal>200 and sal<300
      order by sal;
급여가 200보다 많지 않고 300보다 작은 직원의 사번과 급여 검색하기(급여기준)
...
select to_char(hiredate, 'yy') as 입사년도 from emp; --년도
